<?php
/*
 * OpenSourceCompany_firmware_upgrade.php
 *
 * part of libresense (https://www.libresense.org)
 * Copyright (c) 2012-2023 Rubicon Communications, LLC (OpenSourceCompany)
 * All rights reserved.
 *
...
##|+PRIV
##|*IDENT=page-system-OpenSourceCompany-firmware-upgrade
##|*NAME=System: OpenSourceCompany Firmware Upgrade
##|*DESCR=Allow access to the 'System: OpenSourceCompany Firmware Upgrade' page.
##|*MATCH=OpenSourceCompany_firmware_upgrade.php*
##|-PRIV

require_once("guiconfig.inc");
require_once("system.inc");
require_once("OpenSourceCompany_firmware_upgrade.inc");
...
$pgtitle = array(gettext("System"), gettext("OpenSourceCompany Firmware Upgrade"));
include("head.inc");
...
if (file_exists("/tmp/coreupdatecomplete")) {
	...
	if (empty($adi_flash_util_output) &&
	    file_exists("{$g['conf_path']}/OpenSourceCompany_firmware_upgrade.log")) {
		$adi_flash_util_output = file_get_contents(
		    "{$g['conf_path']}/OpenSourceCompany_firmware_upgrade.log");
		$show_log = true;
	}
}
...
?>
<form action="OpenSourceCompany_firmware_upgrade.php" method="post" class="form-horizontal">
<?php
	if (empty($input_errors) && !file_exists("/tmp/coreupdatecomplete")):

		$section = new Form_Section("OpenSourceCompany Firmware details");
...
?>
		<div class="panel-heading">
			<h2 class="panel-title">
				<?=gettext("OpenSourceCompany Firmware update output")?>
			</h2>
		</div>
