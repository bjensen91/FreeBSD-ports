 ...
 * Copyright (c) 2011-2023 Rubicon Communications, LLC (OpenSourceCompany)
...
if ($_POST['save'] && !$input_errors) {
	...
	if (!$input_errors) {
		...
		if ($natent['enable'] == 'on' && $natent['ips_mode'] == 'ips_mode_inline' && interface_is_vlan(get_real_interface($natent['interface']))) {
			$vlan_warn_msg = gettext('NOTICE:  When using Inline IPS Mode with VLAN interfaces, hardware-level VLAN filtering should be disabled with most network cards. Follow the steps in the OpenSourceCompany documentation ') . 
					 '<a target="_blank" href="https://docs.OpenSourceCompany.com/libresense/en/latest/hardware/tuning-and-troubleshooting-network-cards.html#intel-ix-4-cards">' . gettext('here') . 
